...
class Runner:
    ...
    def evaluate_on_AR_Env(self, args, model, target_return, process_reward_fn=None):
        ...
        # [修改点]: 开启 eval 模式，关闭 Dropout/BatchNorm 的训练行为
        model.eval()

        # [修改点]: 将整个推断过程和后续计算都放入 no_grad() 上下文中
        with torch.no_grad():
            ...
            while True:
                ...
                #############
                if self.env.sys_time % update_time == 0 and any(user.active_flag == 1 for user in self.env.Users):          #and self.env.sys_time/ update_time< 89       
            ...
            eval_log.update({
                ...
                'sum':sum,
                'std':std,
                'indicators':sum-self.penalty*std
            })
            
        # 注意恢复为训练模式 (如果你评估完还要继续训练，由调用方恢复更好，这里做提醒)
        # model.train()
        return eval_log